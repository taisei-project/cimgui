...
local function func_header_impl_generate(FP,stub)
    local outtab = {}
	...
	for _,t in ipairs(FP.funcdefs) do
		if t.cimguiname then
			...
			if def.stname == "" then --ImGui namespace or top level
				table.insert(outtab,"\nstatic inline "..def.ret.." "..def.ov_cimguiname..def.args.." {"..addcoment.."\n")
				if def.ret ~= "void" then
					table.insert(outtab,"    return 0;\n")
				end
				table.insert(outtab,"}")
			else
				cpp2ffi.prtable(def)
...
local function cimgui_generation(parser,stub)
...
--]]
    ...
    local cimplem = func_implementation(parser, false)
    local cimplem_stub = func_implementation(parser, true)

	local ftdef = FREETYPE_GENERATION and "#define IMGUI_ENABLE_FREETYPE\n" or ""

    local hstrfile = read_data"./cimgui_template.cpp"
    content = hstrfile:gsub([[#include "auto_funcs%.cpp"]], cimplem)
    content_stub = hstrfile:gsub([[#include "auto_funcs%.cpp"]], cimplem_stub)

    save_data("./output/cimgui.cpp", cimgui_header, ftdef, content)
    save_data("./output/cimgui_stub.cpp", cimgui_header, ftdef, content_stub)
end
--------------------------------------------------------
-----------------------------do it----------------------
--------------------------------------------------------
